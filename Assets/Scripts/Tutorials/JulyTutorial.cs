...
public class JulyTutorial : MonoBehaviour
{
    //cursor variables
    ...
    [FormerlySerializedAs("izkiModule")] public GameObject sourceModule;
    public GameObject sourceInput;
    [FormerlySerializedAs("izkiOutput")] public GameObject sourceOutput;
    public GameObject currentModule;
    public GameObject triggerOutput;
    ...
    void Update()
    {
        ...
        if (currentStep == 7)
        {
            ...
            cursorStart = new Vector3(sourceModule.transform.position.x, sourceModule.transform.position.y, -6f);
            if (!cursorSet)
            {
                fakeCursorPos = cursorStart;
                cursorSet = true;
            ...
            if (sourceModule.transform.parent == weaponRack.transform) 
            {
                fakeCursor.SetActive(false);
                modulePlaceHighlight.SetActive(false);
                isComplete = true;
        ...
        if (currentStep == 8)
        {
            ...
            isComplete = false;
            cursorSpeed = 3;
            cursorStart = new Vector3(triggerOutput.transform.position.x, sourceOutput.transform.position.y, -6f);
            cursorTarget = new Vector3(sourceInput.transform.position.x, sourceInput.transform.position.y, -6f);
            fakeCursor.SetActive(true);

            tutorialText.text =
                "Weapon modules need to be triggered <br><br> Plug the trigger output into the source's input";
            
            jack1Pos = new Vector3(triggerOutput.transform.position.x, triggerOutput.transform.position.y, -9);
            jack2Pos = new Vector3(funnelInput.transform.position.x, funnelInput.transform.position.y, -7);
            jack1.SetActive(true);
            jack2.SetActive(true);
        ...
        if (currentStep == 9)
        {
            ...
            isComplete = false;
            cursorSpeed = 3;
            cursorStart = new Vector3(sourceOutput.transform.position.x, sourceOutput.transform.position.y, -6f);
            ...
            jack1Pos = new Vector3(sourceOutput.transform.position.x, sourceOutput.transform.position.y, -7);
            jack2Pos = new Vector3(weaponInput.transform.position.x, weaponInput.transform.position.y, -9);
            jack1.SetActive(true);
            jack2.SetActive(true);

            // TODO: find a better way of doing this because i think this will complete before it's even plugged in
            if (sourceModule.GetComponent<Module>().childWires != null)
            {
                fakeCursor.SetActive(false);
                jack1.SetActive(false); 
                jack2.SetActive(false);
